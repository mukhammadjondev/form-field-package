  ...
  ({ className, iconLeft, iconRight, type = 'text', ...props }, ref) => {
    return (
      <div className="relative flex items-center">
        {iconLeft && <span className="absolute left-3 top-1/2 transform -translate-y-1/2">{iconLeft}</span>}
        <input
          type={type}
          ref={ref}
          className={cn(
          ...
          {...props}
        />
        {iconRight && <span className="absolute right-3 top-1/2 transform -translate-y-1/2">{iconRight}</span>}
      </div>
    )
  }
)
